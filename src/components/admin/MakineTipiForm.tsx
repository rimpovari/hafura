...
export function MakineTipiForm({ initialData, isEdit }: { initialData?: any; isEdit?: boolean }) {
  const supabase = createClient();
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [form, setForm] = useState({ makine_tip_kod: initialData?.makine_tip_kod ?? "", makine_tip_adi: initialData?.makine_tip_adi ?? "", aciklama: initialData?.aciklama ?? "", aktif: initialData?.aktif ?? true });

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault(); setLoading(true); setError("");
    if (isEdit) {
      const { error } = await supabase.from("makine_tipi").update(form).eq("makine_tip_id", initialData.makine_tip_id);
      if (error) { setError(error.message); setLoading(false); return; }
    } else {
      const { error } = await supabase.from("makine_tipi").insert({ makine_tip_id: "MT" + Date.now().toString(36).toUpperCase(), ...form });
      if (error) { setError(error.message); setLoading(false); return; }
    }
    router.push("/admin/makine-tipleri"); router.refresh();
  }

  async function handleDelete() {
    if (!confirm("Silmek istediğinizden emin misiniz?")) return;
    await supabase.from("makine_tipi").delete().eq("makine_tip_id", initialData.makine_tip_id);
    router.push("/admin/makine-tipleri"); router.refresh();
  }
  ...
  return (
    <div className="max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-2xl font-bold text-slate-900">{isEdit ? "Makine Tipi Düzenle" : "Yeni Makine Tipi"}</h2>
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-2xl p-8 space-y-5 shadow-sm">
        ...
        <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Kod</label><input required value={form.makine_tip_kod} onChange={(e) => setForm(f => ({ ...f, makine_tip_kod: e.target.value }))} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="örn. EKSKAVATÖR" /></div>
        <div><label className="block text-sm font-medium text-slate-700 mb-1.5">Ad</label><input required value={form.makine_tip_adi} onChange={(e) => setForm(f => ({ ...f, makine_tip_adi: e.target.value }))} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Makine tipi adı" /></div>
